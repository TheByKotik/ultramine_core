...
public final class AsynchronousExecutor<P, T, C, E extends Throwable>
{
	...
	class Task implements Runnable
	{
				...
				finally
				{
					...
					else
					{
						...
						synchronized(this)
						{
							...
							state = STAGE_1_COMPLETE; // They're already
														// synchronized, atomic
														// locks are not needed
						}
						// We want to return false, because we know a
						// synchronous task already handled the finish()
						ret = false; // Don't return inside finally; VERY bad
										// practice.
					}
				}
		...
		void initSync()
		{
			...
			{
				// Async thread is running, but this shouldn't be likely; we
				// need to sync to wait on them because of it.
				synchronized(this)
				{
			...
			{
				// Async thread is not pending, the more likely situation for a
				// task not pending
			}
		}
		...
		T get() throws E
		{
			...
			{
				// 'this' is a placeholder to prevent callbacks from being empty
				// during finish call
			...
			case STAGE_1_COMPLETE:
				try
				{
					...
					for(C callback : callbacks)
					{
						if(callback == this)
						{
							// 'this' is a placeholder to prevent callbacks from
							// being empty on a get() call
		...
		boolean drop()
		{
			...
			{
				// We need the async thread to finish normally to properly
				// dispose of the task
				return false;
			}
		}
	...
	/**
	 * Adds a callback to the parameter provided, adding parameter to the queue
	 * if needed.
	 * <p>
	 ...
	 * execution. Stage 3 will have no callbacks, stage 2 will be skipped unless
	 * a {@link #get(Object)} is used, and stage 1 will be avoided on a
	 * best-effort basis.
	 * <p>
	 ...
	 *         that, no exceptions will be thrown in {@link #finishActive()} for
	 *         the parameter, and {@link #get(Object)} will throw an
